...
%flash Flashes mondriands and stimulus.
    ...
    if obj.stimulus_appearance_delay ~= 0
        ...
        with_stimulus(obj, mask+1, obj.contrasts_in(1), obj.delay)
        obj.results.stimulus_onset = obj.vbl;   
    else
        with_stimulus(obj, mask, obj.contrasts_in(1), obj.delay)
        obj.results.mondrians_onset = obj.vbl;
        obj.results.stimulus_onset = obj.vbl;
    end
    ...
    for n = 2:(1+obj.masks_number_while_fade_in*obj.waitframe)
        with_stimulus(obj, obj.mask_indices_while_fade_in(n), obj.contrasts_in(n), 0)
    end
    ...
    for mask = (1+obj.masks_number_before_stimulus + obj.masks_number_while_fade_in):(obj.cumul_masks_number_before_fade_out-1)
        with_stimulus(obj, mask, obj.contrasts_in(end), obj.delay)
    end

    
    % FADE OUT
    if obj.stimulus_fade_out_duration ~= 0
        % Stimulus
        with_stimulus(obj, obj.cumul_masks_number_before_fade_out, obj.contrasts_out(end-1), obj.delay)
        obj.results.stimulus_fade_out_onset = obj.vbl;   
        
        
        for n = flip(1:(obj.masks_number_while_fade_out*obj.waitframe-1))
            with_stimulus(obj, obj.mask_indices_while_fade_out(n), obj.contrasts_out(n), 0)
        end
    ...
    for mask = (obj.cumul_masks_number_before_fade_out+obj.masks_number_while_fade_out+1):obj.masks_number
        masks_only(obj, mask, obj.delay);
    end
    if (obj.masks_number - (obj.cumul_masks_number_before_fade_out+obj.masks_number_while_fade_out+1)) >= 0
        % Left screen cross
...
function with_stimulus(obj, mask, contrast, delay)
    % Stimulus
    Screen('DrawTexture', obj.window, obj.stimulus, [], obj.stimulus_rect, obj.stimulus_rotation, 1, contrast);
    % Mondrian
    Screen('DrawTexture', obj.window, obj.textures{mask}, [], obj.masks_rect, 0, 1, obj.mask_contrast);
    % Left screen cross
    Screen(obj.fixation_cross_args{1}{:});
