function [x0, y0, x1, y1, i, j] = get_stimulus_rect_shift(alignment, size, padding)
...
% column indicied will have the same X coordinates.
    ...
    [i, j] = find(s == alignment);
    % Calculate X axis coordinates.
    [x0, x1] = coord_shift(j, size, padding);
    % Calculate Y axis coordinates
    [y0, y1] = coord_shift(i, size, padding);
end
