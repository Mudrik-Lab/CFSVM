classdef (Abstract) Stimulus < CFSVM.Element.TemporalElement & CFSVM.Element.SpatialElement
    ...
    methods (Access = protected)
        ...
        function [x_left, y_left, x_right, y_right, ir, ic] = get_stimulus_rect_shift(obj, position)
            ...
            %
            % Returns:
            %   [x_left, y_left, x_right, y_right, ir, ic]: Array containing:
            %
            %   - x_left: Float describing correction to the x0 coordinate.
            %   - y_left: Float describing correction to the y0 coordinate.
            %   - x_right: Flx_leftoat describing correction to the x1 coordinate.
            %   - y_right: Float describing correction to the y1 coordinate.
            ...
            [x_left, x_right] = obj.coord_shift(ic, obj.size, obj.padding);
            % Calculate Y axis coordinates
            [y_left, y_right] = obj.coord_shift(ir, obj.size, obj.padding);
        end
    ...
    methods (Static, Access = protected)
        ...
        function [x, y] = coord_shift(index, size, padding)
            % Calculates actual shifts for the provided index.
            switch index
                case 1
                    x = (0.5 - size) * padding;
                    y = 0.5 - (0.5 - size) * (1 - padding);
                case 2
                    x = (1 - size) / 2;
                    y = (1 + size) / 2;
                case 3
                    x = 0.5 + (0.5 - size) * (1 - padding);
                    y = 1 - (0.5 - size) * padding;
            end
        end
